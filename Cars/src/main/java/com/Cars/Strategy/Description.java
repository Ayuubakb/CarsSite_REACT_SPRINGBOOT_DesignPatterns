...
public class Description implements CarStrategy{
    @Override
    public usedCar createCar(usedCar usedCar, MultipartFile photo, String description, HttpSession session) throws IOException {
        return new usedCar
                .usedCarBuilder(usedCar.getMaker(),
                    usedCar.getModel(), usedCar.getYear(),
                    usedCar.getMileage(), usedCar.getPrice(),
                    ((HashMap<String,String>)session.getAttribute("UserInfo")).get("Login")
                )
